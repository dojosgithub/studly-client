...
const ProjectSubcontractor = () => {
  ...
  const handleSelect = (tradeId, email) => {
    ...
    const hasEmailAndId =
      'email' in filteredSubcontractorByEmail && 'id' in filteredSubcontractorByEmail;
    ...
    //         if(hasEmailAndId){
    //             updatedTrades[existingTradeIndex].subcontractorId =  filteredSubcontractorByEmail._id;
    //         }
          ...
          {trades?.map(({ tradeId, name }) => (
            ...
            >
              ...
              <Box width="100%" maxWidth="200px">
                <Select
                  onChange={(e) => handleSelect(tradeId, e.target.value)}
                  name={tradeId}
                  value={options[tradeId].email}
                  label=""
